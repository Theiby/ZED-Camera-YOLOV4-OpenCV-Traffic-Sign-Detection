...
def main() :
    ...
    while key != 113 :
        ...
        if err == sl.ERROR_CODE.SUCCESS :
            ...
            channel3 = image_ocv[:, :, :-1]

            cv2.imshow("Image", channel3)

            # cv2.imshow("Depth", depth_image_ocv)   uncomment if you want to see depthimage

            key = cv2.waitKey(10)
            return channel3

            process_key_event(zed, key)
            if cv2.setMouseCallback("Image", mouse_callback) != None:
                print(cv2.setMouseCallback("Image", mouse_callback))
